...
//
// A story is { id, title, emoji, scene, beats:[...] }. `scene` is a Monterey scene id
// (mission | bigsur | carmelvalley | pebble | pacificgrove | montereybay) used as the
// painted backdrop; a beat may override it with its own `scene`.
//
// Beat kinds (each beat also has a `say` line that is shown and spoken):
//   { say }                                        → narration only ("Next ▶")
